...
**Share Extension + качественный import статьи/URL.** Это самое важное расширение рынка: человек уже имеет текст в Safari, Reddit, Medium, Substack или другом приложении и отправляет его в PaceFlow. Функция сложнее текущего патча, потому что требует отдельного iOS target, безопасного извлечения статьи, lifecycle-тестов и QA на устройстве. Делать её стоит, если встроенное демо вызывает интерес, но отсутствие удобного входа мешает повторному использованию.
...
Люди, у которых накоплены статьи, newsletters, сохранённые посты и длинные рабочие тексты. Это самый широкий сегмент, но для него нужен Share/URL import.
...
- не менее 3 просят удобный Share/URL/PDF import либо готовы купить lifetime unlock.

Если верх воронки не запускает даже бесплатное демо, App Store-релиз и новые функции не помогут. Если демо запускают, но не возвращаются — проблема в ценности/методе. Если возвращаются, но импорт мешает — тогда Share Extension и PDF получают доказанный приоритет.
...
4. Share/URL import строится только после сигнала повторного использования;
