...
4. **Chrome Quick Send.** Добавлено 10 августа: Manifest V3-расширение отправляет в PaceFlow выделенный, скопированный или вставленный текст, текущую статью или ссылку. Передача одноразовая, живёт в `chrome.storage.session` не более десяти минут, а расширение не просит доступ к истории или ко всем сайтам. Текст автоматически сохраняется локально и запускает focus mode; для статей переиспользуется SSRF-защищённый URL import.
...
4. URL import и Chrome Quick Send проверяют сценарий уже сейчас, а нативный iOS Share Extension строится только после сигнала повторного использования;
