...
async function stopAllEmulators() {
    ...
    await sleep(2000);
    try { execSync('pkill -15 -f qemu 2>/dev/null || true', { encoding: 'utf8', timeout: 5000 }); } catch (e) {}
    try { execSync('pkill -15 -f emulator 2>/dev/null || true', { encoding: 'utf8', timeout: 5000 }); } catch (e) {}
    await sleep(2000);
...
async function launchAVDIfNeeded(avdName) {
    ...
    for (let i = 0; i < 300; i++) {
        ...
        if (devs.length > 0) {
            ...
            const statusSys = runCmd(`adb shell getprop sys.boot_completed`, { allowFail: true }).trim();
            const statusDev = runCmd(`adb shell getprop dev.bootcomplete`, { allowFail: true }).trim();
